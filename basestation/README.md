...
RoboCup MSL（Middle Size League）の試合管制システムとして機能し、RefereeBoxからのゲームコマンドを受信・解析し、5台のプレイヤーロボットへの指令配信、プレイヤーロボットの状態監視、および試合状況の可視化を統合管理します。以下が主な機能になります．
...
- [概要](#概要)
    - [主な機能](#主な機能)
- [クイックスタート](#クイックスタート)
    - [推奨: 統合Launch Package を使用](#推奨-統合launch-package-を使用)
    - [手動起動（トラブルシューティング用）](#手動起動トラブルシューティング用)
- [システムアーキテクチャ](#システムアーキテクチャ)
- [パッケージ一覧](#パッケージ一覧)
    - [動作構成](#動作構成)
- [ディレクトリ構成](#ディレクトリ構成)
    - [重要ファイルの説明](#重要ファイルの説明)
- [パッケージリファレンス](#パッケージリファレンス)
    - [クイックリファレンス：コマンド変換](#クイックリファレンスコマンド変換)
- [最近の実装状況](#最近の実装状況)
- [運用時のベストプラクティス](#運用時のベストプラクティス)
...
basestationは以下の5つの専門化パッケージで構成されています：

| パッケージ | 説明 | 役割 | 通信 |
|-----------|------|------|------|
| 🚀 [musashi_basestation](./musashi_basestation/README.md) | 統合Launch管理 | 各コンポーネントの起動統合・初期設定適用 | ROS2 launch |
